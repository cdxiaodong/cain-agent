...
def test_skill_file_exists():
    """技能文档存在"""
    skill_path = Path("skills/web/command-injection/SKILL.md")
    assert skill_path.exists(), "SKILL.md 不存在"


def test_skill_structure():
    """技能文档结构完整"""
    content = Path("skills/web/command-injection/SKILL.md").read_text(encoding="utf-8")
...
def test_payload_examples():
    ...
    assert "whoami" in content, "缺少 whoami 示例"
    assert "id" in content, "缺少 id 示例"
    assert "hostname" in content, "缺少 hostname 示例"
...
def test_dangerous_commands_filtered():
    """无危险命令示例"""
    content = Path("skills/web/command-injection/SKILL.md").read_text(encoding="utf-8")
    
    dangerous = ["rm -rf", "format", "dd if="]
    for cmd in dangerous:
        assert cmd not in content, f"包含危险命令: {cmd}"
