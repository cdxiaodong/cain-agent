...
def test_contains_benchmark_executor() -> None:
    """文章必须包含 BenchmarkExecutor 或相关类名。"""
    text = _read_article()
    # 检查 BenchmarkExecutor（大小写敏感）
    assert "BenchmarkExecutor" in text, "文章缺少 BenchmarkExecutor 类名"


def test_contains_benchmark_concept() -> None:
    """文章必须包含「基准测试」或「benchmark」相关概念。"""
    text = _read_article()
    # 多种可能的表述方式
    found = (
        "基准测试" in text
...
def test_consistent_terminology() -> None:
    ...
    text = _read_article()
    # 检查核心术语是否多次出现
    key_terms = ("基准", "指标", "靶场", "评估", "漏洞", "检测")
    for term in key_terms:
        # 每个核心术语至少出现 3 次
        count = text.count(term)
        if count > 0:  # 如果出现，应该有一定频率
